...
function TrabajoCardQR({trabajo}) {
    ...
    const {guardarTrabajoID}= trabajoAuxContext;

    const {_id,titulo}= trabajo;

    const guardarTrabajoIDA =(id)=>{
        guardarTrabajoID(id);
    }
  return (
    <div className='grid grid-rows-2 m-2 h-24  bg-blue-800 items-center relative shadow-lg rounded-sm w-11/12'>
          ...
          <div className='ml-4'>
            <button className='mr-2 px-2 text-white bg-green-700 rounded-sm shadow-md text-sm' onClick={()=>guardarTrabajoIDA(_id)}>Crear QR</button>
          </div>
